...
python3 test.py -r packet.txt -c

4) Full scanning (params + cookies + header probes):
python3 test.py -r packet.txt -f

5) Fast modes:
python3 test.py -r packet.txt -faster
python3 test.py -r packet.txt -fastest
python3 test.py -r packet.txt -fastest -f

6) Scan a single URL:
python3 test.py -u "https://example.com/page?id=1"

7) Scan a list of URLs:
python3 test.py -ul urls.txt
...
### Example usage (2)
![Example usage - testing full mode on top of testing for the prams it will check for cookies -> UserAgent + Referer + X-Forwarded-For](2.png)
